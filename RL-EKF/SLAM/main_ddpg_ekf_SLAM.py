...
if __name__ == '__main__':
    deltaT = .02
    # robot-dependent motion noise parameters
    alphas = np.array([.1, .01, .18, .08, 0, 0])  
    ...
    figure_file = 'plots/DDPG-EKF-SLAM-Reward.png'
    best_score = 0
    score_history = []
...
#            agent.save_models()
        ...
        episode += 1
        print('episode ', episode, 'score %.5f' % score, 'step', step)
        
    avg_loss = path_loss(start_training_step, end_training_step, robot_groundtruth, Robot_Est=env.Robot_Est)
    print('average path loss for training: ', avg_loss)
    
    np.save('robot_estimate_rl_ekf_SLAM.npy',env.Robot_Est)
